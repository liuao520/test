...
class GroundTruth(object):
    ...
    def get_ground_truth(self):
        ...
        batch_images_tensor = flow.stack(values=batch_images, axis=0)    # (batch_size, image_height, image_width, channels)
        batch_target_tensor = flow.stack(values=batch_target, axis=0)    # (batch_size, heatmap_height, heatmap_width, num_of_joints)
        batch_target_weight_tensor = flow.stack(values=batch_target_weight, axis=0)  # (batch_size, num_of_joints, 1)
    ...
    def __get_one_human_instance_keypoints(self, line_keypoints):
        ...
        keypoints_tensor = flow.tensor_buffer_to_tensor(keypoints, flow.float32)
        keypoints_tensor = flow.reshape(keypoints_tensor, shape=(-1, 3))
    ...
    def __get_keypoints_3d(self, keypoints):
        ...
        for i in range(self.num_of_joints):
            ...
            keypoints_3d_list.append(flow.tensor_buffer_to_tensor([keypoints[i, 0], keypoints[i, 1], 0], flow.float32))
            ...
            keypoints_3d_list.append(flow.tensor_buffer_to_tensor([exist_value, exist_value, 0], flow.float32))

        keypoints_3d = flow.stack(values=keypoints_3d_list, axis=0)  # shape: (self.num_of_joints, 3)
        keypoints_3d_exist = flow.stack(values=keypoints_3d_exist_list, axis=0)   # shape: (self.num_of_joints, 3)
    ...
    def __generate_target(self, keypoints_3d, keypoints_3d_exist):
        ...
        target = flow.tensor_buffer_to_tensor(target, flow.float32)
        target = flow.transpose(a=target, perm=[1, 2, 0])    # shape : (self.heatmap_size[0], self.heatmap_size[1], self.num_of_joints)
        # target_weight = tf.convert_to_t?ensor(target_weight, tf.dtypes.float32)    # shape : (self.num_of_joints, 1)
        target_weight = flow.tensor_buffer_to_tensor(target_weight, flow.float32)
